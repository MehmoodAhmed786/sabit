...
function startOfDayISO(d: Date) { return new Date(d.getFullYear(), d.getMonth(), d.getDate()).toISOString().slice(0,10) }
...
export default function Progress() {
  ...
  const [qadaTotals, setQadaTotals] = useState<{ pending: number; completed: number } | null>(null)
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // streaks
      const s = await supabase.from('streaks').select('current_streak,longest_streak').eq('user_id', uid).single()
      if (s.error && s.status !== 406) console.debug('streaks fetch error', s.error)
      setStreaks(s.data || { current_streak: 0, longest_streak: 0 })

      // qada totals
      const qPending = await supabase.from('qada_records').select('id', { count: 'exact', head: true }).eq('user_id', uid).eq('status', 'pending')
      const qCompleted = await supabase.from('qada_records').select('id', { count: 'exact', head: true }).eq('user_id', uid).eq('status', 'made_up')
      setQadaTotals({ pending: qPending.count || 0, completed: qCompleted.count || 0 })

      // prayer records for a reasonable window (we'll fetch all and filter client-side for flexibility)
      const pr = await supabase.from('prayer_records').select('id,user_id,date,prayer_name,status').eq('user_id', uid)
  ...
  useEffect(() => {
    fetchAll()
    const ch = supabase.channel('realtime-progress').on('postgres_changes', { event: '*', schema: 'public', table: 'prayer_records' }, () => {
      fetchAll()
    }).subscribe()
  ...
  const periodRange = useMemo(() => {
    ...
    const start = new Date(today); start.setDate(start.getDate() - (p.days! - 1))
    return { start: startOfDayISO(start), end: startOfDayISO(today) }
  }, [period])

  const filteredRecords = useMemo(() => {
    if (!records) return []
    if (!periodRange.start) return records
    return records.filter(r => r.date >= periodRange.start! && r.date <= periodRange.end)
  }, [records, periodRange])

  // days in range (for expected prayers)
  const daysCount = useMemo(() => {
    if (!periodRange.start) return (() => {
      // determine from earliest record
  ...
  const completionPct = totalExpected ? Math.round((completedCount / totalExpected) * 1000) / 10 : 100

  // prayer-by-prayer rates
  const prayerRates = useMemo(() => {
    const names = ['Fajr','Dhuhr','Asr','Maghrib','Isha']
    return names.map(name => {
      const totalDays = daysCount
      const completed = filteredRecords.filter(r => r.prayer_name.toLowerCase() === name.toLowerCase() && r.status === 'completed').length
      const pct = totalDays ? Math.round((completed / totalDays) * 1000) / 10 : 0
      return { name, pct, completed }
    })
  }, [filteredRecords, daysCount])
  ...
  const calendarData = useMemo(() => {
    ...
    for (let d=1; d<=daysInMonth; d++) map.set(`${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`, 0)
  ...
  const personalRecords = useMemo(() => {
    ...
    const totalCompletedAll = records ? records.filter(r => r.status === 'completed').length : 0
    const totalQadaCompleted = qadaTotals?.completed ?? 0
    // most consistent month: compute percent per month
    ...
    let bestMonth = null as string | null, bestPct = 0
    ...
    return { longest, totalCompletedAll, totalQadaCompleted, bestMonth, bestPct: Math.round(bestPct*10)/10 }
  }, [records, streaks, qadaTotals])

  if (loading) return <div className="page-content">Loading your progress…</div>
  if (error) return <div className="page-content">Error loading progress: {error} <button onClick={fetchAll}>Retry</button></div>
  ...
  return (
    <div className="page-content">
      ...
      <section style={{marginTop:12}}>
        <div style={{display:'flex', gap:12}}>
          ...
          <div style={{width:160, background:'#fff', padding:12, borderRadius:10}}>
            ...
            <div style={{fontSize:20, fontWeight:700}}>{qadaTotals?.pending ?? 0}</div>
      ...
      <section style={{marginTop:16}}>
        ...
        <div style={{display:'flex', gap:12, marginTop:8}}>
          ...
          <div style={{flex:1, background:'#fff', padding:12, borderRadius:10}}>
            ...
            <div style={{fontSize:20, fontWeight:700}}>{missedCount}</div>
          </div>
          <div style={{flex:1, background:'#fff', padding:12, borderRadius:10}}>
            <div className="muted">Completion</div>
            <div style={{fontSize:20, fontWeight:700}}>{completionPct}%</div>
          </div>
        </div>
      ...
      <section style={{marginTop:16}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
          <h3>Prayer Calendar — {calendarMonth.toLocaleString(undefined,{month:'long', year:'numeric'})}</h3>
          <div style={{display:'flex', gap:8}}>
            <button type="button" onClick={() => setCalendarMonth(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))}>←</button>
            <button type="button" onClick={() => setCalendarMonth(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))}>→</button>
          </div>
        </div>
        <div style={{display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:6}}>
          {Array.from({length: calendarData.daysInMonth}).map((_,i) => {
            const d = i+1
            const key = `${calendarMonth.getFullYear()}-${String(calendarMonth.getMonth()+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`
